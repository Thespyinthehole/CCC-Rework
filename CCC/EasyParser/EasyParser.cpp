...
{
    first_sets.empty();
    std::map<ParseTree (*)(), ParseTree>::iterator names_iter = recursive_trees.begin();
    std::map<ParseTree (*)(), ParseTree>::iterator names_end = recursive_trees.end();
...
void EasyParser::register_tree(ParseTree (*function)())
{
...
{
    on_complete.push_back(complete);
    return *this;
}
...
int ParseTree::evaluate(std::vector<Token> &tokens, int offset, bool testing, EasyParser &parser)
{
    ...
    case or_tokens:
    {
        ...
        for (int i = 0; i < other_trees.size() && !called; i++)
        {
            ...
            if (first.find(next_token.token) != first.end() || first.find(INT_MIN) != first.end())
            {
                offset = other_trees[i].evaluate(tokens, offset, testing, parser);
                called = true;
            }
    ...
    if (!testing)
        for (int i = 0; i < on_complete.size(); i++)
            on_complete[i]();
    return offset;
}
...
/**
 std::map<ParseTree (*)(), std::set<int>>::iterator iter = first_sets.begin();
    std::map<ParseTree (*)(), std::se|t<int>>::iterator end = first_sets.end();
